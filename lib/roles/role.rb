  ...
  module Role
    ...
    module ClassMethods
      def with_role(role_name, resource = nil)
        if resource.nil?
          ...
          self.includes(role_table).where("#{role_table}.name LIKE '%s'", role_name.to_s).where("#{role_table}.resource_type LIKE '%s'", resource.class.base_class.to_s).where("#{role_table}.resource_id = %s", resource.id)
        end
      end
    end
    ...
    def add_role(role_name, resource = nil)
      if resource.nil?
        ...
        self.send(self.class.role_table).where(:resource_type => resource.class.base_class.to_s).where(:resource_id => resource.id).where(:name => role_name.to_s).first_or_create
      end
    end
    ...
    def has_role?(role_name, resource = nil)
      if new_record?
        ...
        if resource.nil?
          ...
          self.send(self.class.role_table).where(:name => role_name.to_s).where(:resource_type => resource.class.base_class.to_s).where(:resource_id => resource.id).size > 0
        end
      end
    end
    ...
    def remove_role(role_name, resource = nil)
      if resource.nil?
        ...
        self.send(self.class.role_table).where(:name => role_name.to_s).where(:resource_type => resource.class.base_class.to_s).where(:resource_id => resource.id).destroy_all
      end
    end
    ...
    def role_names(resource = nil)
      if resource.nil?
        ...
        self.send(self.class.role_table).where(:resource_type => resource.class.base_class.to_s).where(:resource_id => resource.id).select(:name).map { |r| r.name }
      end
    end
